...
Likewise, if an event is certain not not to occur, it is assigned a probability $0$.

In order to make future discussions more concise I define two concepts related to the configuration of a probabilistic system: /state/ and /sample space/.
...
Given that $P(A,B) = P(A)P(B) = P(B)P(A) = P(B,A)$, we have $P(A,B) = P(B,A) = P(B|A)P(A)$, which leads us to a powerful result in probability: *Bayes Theorem* cite:bayesessay.
...
An observing program is estabilised to analyse transient signals detected with one or both of these telescopes, with the belief that abbr:gw bursts can be produced by either a abbr:bns coalesence, or a abbr:bbh coalesence.
...
/Rejection sampling/ uses a similar principle to importance sampling, using a /proposal distribution/, $Q(x)$, which can be sampled directly, to generate the samples (see figure ref:fig:probability:importance-sampling for an illustration of how $P$ and $Q$ relate).
