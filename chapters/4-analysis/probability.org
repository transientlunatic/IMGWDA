...
In many problems the posterior probability distribution which we need to evaluate will be non-analytical, and drawing samples from it will be difficult.
This problem is often further complicated by having access only to the likelihood and prior distributions, and not the evidence. 
