...
Abbr:bbh coalescences are described by a number of physical parameters: the ratio of the two component black holes' masses, $q$; the vector of each component's spin, ${\vec{s}}_1$ and ${\vec{s}}_2$ (with $\hat{s}_{1} = \hat{s}_{2} = \hat{L}$ for \vec{L} the total angular momentum vector for the system: see figure ref:fig:heron:bbhcartoon for the arrangement of these vectors); and the time, $t$, relative to a fixed reference time, for example the time of coalescence of the binary.
...
\caption[Mismatches between Heron, Georgia Tech waveforms, and analytical approximants from in-sample testing]{The distributions of mismatches between waveforms from the Heron model and each of the \gls{nr} waveforms from the Georgia Tech waveform catalogue (black outline histogram) used in the training set using the procedure described in section \ref{sec:heron:testing:in}.
...
\caption[Mismatches of the Heron model compared to the distance to near-by training points]{The mismatch between waveforms produced by the Heron model and \gls{nr} waveforms compared to the distance to the next-nearest training waveform in the training data.
...
\caption[Mismatches between Heron, Georgia Tech waveforms, and analytical approximants from leave-one-out sampling]{The distributions of mismatches between waveforms from the Heron model and each of the \gls{nr} waveforms from the Georgia Tech waveform catalogue (black outline histogram) used in the training set using the leave-one-out testing procedure detailed in section \ref{sec:heron:testing:out}.
Additionally, the mismatch distributions between waveforms produced at the same parameters as the \gls{nr} waveforms by the \texttt{SEOBNRv3} (red outline histogram), and the \texttt{IMRPhenomPv2} (blue outline histogram) phenomenological waveform models are plotted. 
...
\caption[Corner plot of leave-one-out mismatches between Heron and the Georgia Tech catalogue]{A corner plot across the parameter space of the Georgia Tech waveform catalogue showing the leave-one-out mismatch, as described in section \ref{sec:heron:testing:out}.
...
\caption[Corner plot of mismatches between Heron and the SXS catalogue]{A corner plot across the parameter space of the SXS waveform catalogue showing the leave-one-out mismatch, as described in section \ref{sec:heron:testing:out}.
 \label{fig:heron:testing:sxs:tri}}
\end{figure}

\begin{figure}
\includegraphics{figures/heron/sxs-sample-hist.pdf}
\caption[Corner plot of leave-one-out mismatches between Heron and the SXS catalogue]{The distributions of mismatches between waveforms from the Heron model and each of the \gls{nr} waveforms from the SXS waveform catalogue (black outline histogram) using the testing procedure detailed in section \ref{sec:heron:testing:sxs}.
Additionally, the mismatch distributions between waveforms produced at the same parameters as the \gls{nr} waveforms by the \texttt{SEOBNRv3} (red outline histogram), and the \texttt{IMRPhenomPv2} (blue outline histogram) phenomenological waveform models are plotted. 
...
The differences between the phenomenological model and the \ac{GPR} model waveforms are seen to also exist between the phenomenological model waveforms and the \ac{NR}-derived waveform (\texttt{GT0374}), plotted here in green. 
In the right panel the distribution of mismatches between the samples and both phenomenological waveforms are shown, with the vertical lines representing the mismatch between the \ac{GPR} and the phenomenological waveform.
