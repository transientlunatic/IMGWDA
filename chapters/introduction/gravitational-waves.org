...
\end{tikzpicture}
\caption{The effect of a $+$-polarised abbr:gw on a circle of test particles as it propogates through the page (orthogonal to the ring). Time progresses horizontally along the $x$-axis from left to right.
...
\begin{figure}[h]
 ...
 \end{tikzpicture}
\caption{The effect of a $\times$-polarised abbr:gw on a circle of test particles as it propogates through the page (orthogonal to the ring).  Time progresses horizontally along the $x$-axis from left to right.
...
\label{eq:intro:gr:quadrupole2strain}
  h_{jk} = \frac{2}{r} \frac{\dd^2 Q_{jk}}{\dd t^2}
\end{equation}
...
\caption{The abpl:asd for the first generation of large-scale interferometers: initial gls:ligo (red), gls:virgo (blue), gls:tama (grey), and GEO600 (purple). \label{fig:detectors:interferometers:firstgen}}
\end{figure}
...
The $+$-polarised response is plotted in figure ref:fig:detectors:interferometers:antennapattern, which clearly depicts the four regions of low sensitivity. 
    ...
    \caption{The antenna pattern, in response to $+$-polarised gravitational waves, of a two-armed interferometric detector with a $90^{\circ}$ arm separation.     \label{fig:detectors:interferometers:antennapattern}}
    \end{figure}
...
\bottomrule
\end{tabular}
\caption{The principle seismic noise frequency bands, $f$, which affect ground-based detectors, their sources, and the distance, $D$, over which the band affects advanced-generation detectors. \label{tab:detectors:noise:seismic}}
\end{table}
...
** Observing run 1 and GW150914
 ...
 \caption{The data from the advanced gls:ligo detectors at the Livingston (L1) and Hanford (H1) observatories, which has been band-passed between $\SI{50}{\hertz}$ and $\SI{250}{\hertz}$, and a comb filter has been applied to remove the $\SI{60}{\hertz}$ line and its higher harmonics.
...
The event itself, a gls:bbh coalescence between a $36^{+5}_{-4} \msun$ black hole and a $29^{+4}_{-4}$ black hole was unexpected.
