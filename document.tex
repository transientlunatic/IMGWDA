...
This document was built with version \input{git-info} of the git repository, and has been assigned the identifier LIGO-P1900001 by the LIGO Document Control Centre.
...
\chapterprecis{
  This chapter discusses the astrophysical sources of gravitational waves, and the form that the gravitational waves take, which we expect to detect as signals.

  This chapter contains a mixture of introductory and novel material, with the majority of the discussion of signals and sources being introductory, with some novel material on the detectability of signals which originate from close-encounters between compact objects, which is discussed in section \ref{sources:burst:encounters:waveforms}.
  Later this chapter contains discussion of how the sensitivity of searches for gravitational wave signals are characterised, which leads into the introduction of the Minke software library in section \ref{sec:sources:minke}.
  The work on Minke is novel, and was essential to the LIGO / Virgo collaboration work briefly discussed in section \ref{sec:sources:burstresults}.
  ...
  Section \ref{sec:sources:mdc} discusses mock data challenges, and how these are used to characterise the algorithms discussed in section \ref{sec:sources:burstsearch}.

  Section \ref{sec:sources:minke} introduces Minke, a novel software library for producing these mock data challenges.
...
\chapterprecis{
  In this chapter I introduce the use of Gaussian process regression as a surrogate modelling method for \gls{gw} waveforms from compact binary coalesensce events.
  Some of the work presented in this chapter has previously been made available on the ArXiv \cite{2019arXiv190309204W}, and at the time of writing are under review for publication. Additional results are presented, specifically related to the comparison between 
